...
public class DelServlet extends HttpServlet {
    ...
    private String obrisi(HttpSession hSesija) {
        ...
        try {
            ...
            String upit = ""; 
            String braut = ""; // broj autora ( kolona au_id, tabela author )
            boolean dodata_kol = false; // da li je ime kolone dodato upitu 
            DetermineID brAutObj = new DetermineID(); // koristi se za pozivanje metoda odrediBrAut( author, stmt ) 
            
            // DETERMINING the AUTHOR ID 
            if (!((preth_autor.equalsIgnoreCase("")))) {
                // odrediBrAut : formiranje i izvršavanje upita SELECT au_id FROM author WHERE au_name='...';
            ...
            if (!(braut.equalsIgnoreCase(""))) { // ako ID autora postoji
