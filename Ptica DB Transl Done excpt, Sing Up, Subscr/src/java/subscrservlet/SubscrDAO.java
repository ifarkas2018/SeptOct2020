...
public class SubscrDAO {
    ...
    // vraća "postoji" ako email već postoji u tabeli subscription inače vraća TRUE ako je došlo do izuzetka, a vraća FALSE ako nije 
    // došlo do izuzetka
    public static String addEmail(String newslEmail) {
        ...
        try {
            ...
            String upit = "select * from subscription where email='" + newslEmail + "';";
            rs = stmt.executeQuery(upit);
            if (!(rs.next())) { // ako email NE POSTOJI u bazi dodaj ga    
                PreparedStatement preparedStmt;
                // 2. insert upit
                upit = "insert into subscription(email) values ('" + newslEmail + "');"; 
